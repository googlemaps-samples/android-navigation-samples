...
/**
 * This shows how to use setPadding to allow overlays that obscure part of the map without obscuring
 * the map UI or copyright notices.
 */
public class VisibleRegionDemoActivity extends AppCompatActivity
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getIntent()
        .getBooleanExtra(
            ...
            /* defaultValue= */ false)) {
      setContentView(R.layout.visible_region_demo_nav_flavor);
      ...
      // Disable header and ETA card, otherwise top and bottom padding would not take effect.
      navFragment.setHeaderEnabled(false);
      navFragment.setEtaCardEnabled(false);
      new OnMapAndViewReadyListener(navFragment, this);
    } else {
      setContentView(R.layout.visible_region_demo_maps_flavor);
      messageView = (TextView) findViewById(R.id.message_text);
  ...
  public void onMapReady(GoogleMap map) {
    ...
    this.map.setPadding(currentLeft, currentTop, currentRight, currentBottom);
    this.map.moveCamera(CameraUpdateFactory.newLatLngZoom(SFO, 18));
    // Add a marker to the Opera House.
    this.map.addMarker(new MarkerOptions().position(SOH).title("Sydney Opera House"));
    // Add a camera change listener.
    this.map.setOnCameraMoveStartedListener(
        new OnCameraMoveStartedListener() {
          @Override
  ...
  public void moveToOperaHouse(View view) {
    ...
    map.moveCamera(CameraUpdateFactory.newLatLngZoom(SOH, 16));
  }
  ...
  public void moveToAUS(View view) {
    ...
    map.moveCamera(CameraUpdateFactory.newLatLngBounds(AUS, 0));
  }
