...
/** This shows how to close the info window when the currently selected marker is retapped. */
public class MarkerCloseInfoWindowOnRetapDemoActivity extends AppCompatActivity
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getIntent()
        .getBooleanExtra(
            ...
            /* defaultValue= */ false)) {
      setContentView(R.layout.marker_close_info_window_on_retap_demo_nav_flavor);
      ...
      setContentView(R.layout.marker_close_info_window_on_retap_demo_maps_flavor);
      setMarginForEdgeToEdgeSupport();
  ...
  public void onMapReady(GoogleMap map) {
    ...
    map.setContentDescription(
        "Demo showing how to close the info window when the currently"
            + " selected marker is retapped.");
  ...
  private void addMarkersToMap() {
    ...
    map.addMarker(
        new MarkerOptions().position(SYDNEY).title("Sydney").snippet("Population: 4,627,300"));
    ...
    map.addMarker(
        new MarkerOptions().position(PERTH).title("Perth").snippet("Population: 1,738,800"));
  ...
  public boolean onMarkerClick(final Marker marker) {
    // The user has retapped on the marker which was already showing an info window.
