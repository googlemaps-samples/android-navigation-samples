...
/** Fragment with "stroke pattern" UI controls for polygons, to be used in ViewPager. */
public class PolygonPatternControlFragment extends PolygonControlFragment
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle bundle) {
    View view = inflater.inflate(R.layout.pattern_control_fragment, container, false);
    patternTextView = (TextView) view.findViewById(R.id.patternTextView);

    patternSolidBtn = (Button) view.findViewById(R.id.patternSolidBtn);
    patternDottedBtn = (Button) view.findViewById(R.id.patternDottedBtn);
    patternDashedBtn = (Button) view.findViewById(R.id.patternDashedBtn);
    patternMixedBtn = (Button) view.findViewById(R.id.patternMixedBtn);
  ...
  public void onClick(View view) {
    ...
    if (id == R.id.patternSolidBtn) {
      ...
      pattern = Arrays.<PatternItem>asList(new Dot(), new Gap(RANDOM.nextFloat() * MAX_GAP_LENGTH));
    } else if (id == R.id.patternDashedBtn) {
      pattern =
          Arrays.<PatternItem>asList(
  ...
  public void refresh() {
    ...
    patternTextView.setText((polygon == null) ? "" : toString(polygon.getStrokePattern()));
  }
  ...
  private static String toString(List<PatternItem> pattern) {
    ...
    for (PatternItem item : pattern) {
      if (item instanceof Dot) {
        ...
        sb.append(String.format(Locale.getDefault(), "%.1fpx-DASH", ((Dash) item).length));
      } else if (item instanceof Gap) {
        sb.append(String.format(Locale.getDefault(), "%.1fpx-GAP", ((Gap) item).length));
      }
      sb.append(" ");
    }
    sb.append("<repeat>");
  ...
  public void onDestroyView() {
    patternSolidBtn.setOnClickListener(null);
    patternDottedBtn.setOnClickListener(null);
    patternDashedBtn.setOnClickListener(null);
    patternMixedBtn.setOnClickListener(null);

    patternSolidBtn = null;
    patternDottedBtn = null;
    patternDashedBtn = null;
    patternMixedBtn = null;
